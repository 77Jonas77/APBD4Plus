...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            //IEnumerable<Emp> result = from emp in Emp where emp.Job == "Backend programmer" select emp;
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result =
                Depts.GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emp) =>
                        new
                        {
                            name = dept.Dname,
                            numOfEmployees = emp.Count()
                        })
                    .Where(g => g.numOfEmployees > 1).Select(g => new
                    {
                        name = g.name,
        ...
        {
            IEnumerable<Emp> result = CustomExtensionMethods.MyExtension(Emps);
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            {
                Department = dept,
                NumOfEmpl = emps.Count()
            }).Where(x => x.NumOfEmpl == 0 || x.NumOfEmpl == 5).OrderBy(x => x.Department.Dname).Select(x => x.Department);
            //result =
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            return emps.Where(emp1 => emps.Any(emp2 => emp2.Mgr?.Empno == emp1.Empno)).OrderBy(e => e.Ename)
